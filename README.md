# Sanjay Kankamwar  
**Software Engineer specializing in Full-Stack Development, Cloud Infrastructure, and Production-Grade Systems**

---

## Technical Expertise  
- **Programming Languages:** Python, JavaScript, TypeScript, HTML/CSS, Node.js  
- **Frameworks & Libraries:** Flask, Django, Next.js, Express.js  
- **Databases:** PostgreSQL, MongoDB, MySQL, Supabase  
- **DevOps & Cloud:** AWS, Docker, Jenkins, Kubernetes, CI/CD Pipelines, Render Platform  
- **Developer Tools:** Git/GitHub, VS Code, Jupyter Notebook  

---

## Professional Experience  

**Software Engineer – Raam Group**  
*June 2025 – Present | Hyderabad, India*  

- Design, develop, and deploy responsive web applications with complete Software Development Life Cycle ownership  
- Built and maintain production CRM and HR management systems serving multiple automotive dealerships  
- Execute large-scale data migration projects ensuring seamless transitions and data integrity across systems  
- Automate lead management workflows for dealership operations using modern web technologies  
- Collaborate with cross-functional teams to deliver high-quality software solutions  

---

## Featured Projects  

### EpicLeads CRM System  
Live at: [www.epicleads.in](http://www.epicleads.in)  
**Tech Stack:** PostgreSQL, Supabase, Flask, HTML, CSS, JavaScript  

A comprehensive lead management system for automotive dealerships with real-time lead tracking and automated workflows. Deployed successfully at Ather Chennai, Ather Hyderabad, and Honda Hyderabad with **99% uptime**.  

**Key Achievements:**  
- Reduced lead response time by **60%** through automation and intelligent routing  
- Implemented secure authentication and role-based access control  
- Built detailed analytics dashboards for actionable insights  
- Streamlined conversion processes improving acquisition rates  

---

### EpicHR Management System  
Live at: [epichr.netlify.app](https://epichr.netlify.app)  
**Tech Stack:** PostgreSQL, Supabase, Django, TypeScript, Next.js  

A full-stack HR management platform covering payroll, attendance, and performance tracking. Built with modern TypeScript + Next.js ensuring seamless UX across devices.  

**Key Features:**  
- Automated HR reporting & data visualization  
- Employee lifecycle management: recruitment, onboarding, performance reviews  
- CI/CD deployment on Render serving active Raam Group users  
- Responsive design optimized for desktop, tablet & mobile  

---

## Education  

**Bachelor of Technology in Information Technology**  
Vishwakarma Institute of Information Technology, Pune  
*Current CGPA: 7.92 | Expected Graduation: August 2026*  

**Relevant Coursework:** IBM Full Stack Development, DevOps, Cloud Computing, Database Management Systems, Human Computer Interaction  

---

## Professional Certifications  
- IBM Full Stack Software Developer Professional Certificate – *Coursera, 2024*  
- AWS Academy Graduate – *Cloud Foundations, AWS Academy 2024*  
- Introduction to Containers with Docker, Kubernetes & OpenShift – *Coursera, 2024*  

---

## Areas of Interest  
Full-stack web development · Cloud-native systems · DevOps automation · Database optimization · API integrations · Scalable infrastructure design  

---

## Contact Information  
**Email:** [sanjaykankamwar@gmail.com](mailto:sanjaykankamwar@gmail.com)  
**LinkedIn:** [linkedin.com/in/sanjaykankamwar](https://www.linkedin.com/in/sanjaykankamwar)  
**GitHub:** [github.com/SanjayKankamwar](https://github.com/SanjayKankamwar)  
